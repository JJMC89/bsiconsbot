...
def process_local_config(config: ConfigJSONObject) -> LocalConfig:
    ...
    return LocalConfig(
        ...
        summary_prefix=config.summaryi_prefix("", require_string),
    )
...
def process_global_config(
    ...
    blacklist = config.blacklist(set(), require_list, GenToPages(site))
    whitelist = config.whitelist(set(), require_list, GenToPages(site))
    bsicons_map = {}
    gen: Iterable[pywikibot.Page] = chain()
    for page in redirects - (blacklist - whitelist):
...
def process_site_config(
    ...
    site_config = SiteConfig(
        bs_templates=config.bs_tempaltes(ToBSTemplatesConfig(site)),
...
class BSiconsReplacer(
    ...
    def site_config(self) -> Optional[SiteConfig]:
        ...
        try:
            ...
            config_dict = copy.deepcopy(self.opt.config).update(config_dict)
            config_json = jsoncfg.loads_config(json.dumps(config_dict))
