...
def main(*args: str) -> int:
    ...
    parser.add_argument(
        ...
        default=1000,
        type=int,
        help="lower bound (non-inclusive) for large files",
    )
    parser.add_argument(
        "--date",
        default=site.server_time().date() - datetime.timedelta(days=1),
        type=datetime.date.fromisoformat,
        help="list changes/logs on this date",
        metavar="YYYY-MM-DD",
    )
    parser.add_argument(
        "--enable",
    ...
    file_redirects = pywikibot.pagegenerators.MySQLPageGenerator(  # nosec
        ...
        "order by page_title".format(
            prefix=BSiconPage.PREFIX,
            suffix=BSiconPage.SUFFIX,
        ),
        site=site,
    )
    save_list(
        file_redirects,
        parsed_args.redirects_page,
    )
    ...
    large_files = pywikibot.pagegenerators.MySQLPageGenerator(  # nosec
        ...
        "order by img_name".format(
            prefix=BSiconPage.PREFIX,
            suffix=BSiconPage.SUFFIX,
            large_size=parsed_args.large_size,
        ),
        site=site,
    )
